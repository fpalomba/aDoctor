...
import it.aDoctor.parser.ClassParser;
import it.aDoctor.parser.CodeParser;
import it.aDoctor.parser.InnerClassVisitor;
				...
				if (subDir.isDirectory()) {
					...
					if (javaFiles.size() > 0) {
						for (File javaFile : javaFiles) {
							...
							try {
								parsed = codeParser.createParser(FileUtilities.readFile(javaFile.getAbsolutePath()));
